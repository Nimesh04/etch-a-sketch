...
function coloredFun(){
    ...
    colored.style.background = "linear-gradient(45deg,rgb(148, 68, 170) 0%,rgb(224, 76, 40) 100%)";
...
function paintSquare(element){
    if(shiftKeyPressed){
        ...
        if(window.getComputedStyle(element).backgroundColor == EMPTY_COLOR){
            if(coloredOption) element.style.backgroundColor = randomColor(0.1);
            if(monotoneOption) element.style.backgroundColor = `rgba(0, 0, 0, 0.1)`;
        }else{
